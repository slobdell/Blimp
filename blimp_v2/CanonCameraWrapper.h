...
class CanonCameraWrapper{
 ...
 //---------------------------------------------------------------------
    ...
    bool grabPixelsFromLiveView(int rotateByNTimes90, int &outDataSize, unsigned char** jpegData, EdsSize &outJpegSize); //capture the live view to rgb pixel array
    bool saveImageFromLiveView(std::string saveName);
